...
This is corroborated by a simple OLS regression model that uses whether a driver had an accident or not as the target and the personality factors as its features. I could go down the non-linear modeling route, but we risk introducing more complexity into our analysis. Logistic regression is preferred because its results are easier to interpret for a client than (say) a boosted decision tree or a neural network.

**Time for some Data Science**

Are there any relationships between driver personality and driving behavior, specifically how fast a driver goes? The sample contains the number of times each driver speeds. However, as mentioned above, the greater the distance driven, the greater the likelihood of these speeding events occuring as well as an increased likelihood of an accident.

speeding event data for each driver but since the number of accidents will be correlated with the distance driven, it makes sens
