...
The ROC curve an easy way to visualize the performance of a binary classifier. The x-axis represents the false positive rate (% times the model predicts Class 1 when the Class is 0) while the y-axis represents the true positive rate (ability to accurately predict a class). We want to maximize the area under the ROC curve which occurs at the top left of Figure 7. Here the True positive rate is 1 and the false positive rate is 0. An AUC of 1.0 is a perfect model, where the classifier never makes a mistake.
...
For the training set The area under this Logistic Classifier based ROC curve is 79%, which means we make a mistake 21% of the time when classifying clients as 1 or 0. This is better than random guessing or employing the Dummy Classifier. Running a 10 fold cross-validation across the sample, drops the area under the curve to 76.1%. 
