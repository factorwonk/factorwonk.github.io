...
I decided to use a Logistic Classifier as it transforms an underlying linear model using a (sigmoid) function to provide a binary outcome. When we identify features with the highest explanatory power for example the is more intuitive than a Random Forest Classifier. 
