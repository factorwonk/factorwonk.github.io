...
The repository containing code for both sections is [here](https://github.com/factorwonk/bcgdv) and the presentation is available in pdf format [here](https://factorwonk.github.io/bcgdv.pdf) and as powerpoint slides [here](https://factorwonk.github.io/bcgdv.pptx).
...
This ROC curve an easy way to visualize the performance of a binary classifier. The x-axis of the ROC represents the false positive rate (% times the model predicts Class 1 when the Class is 0) while the y-axis represents the true positive rate (ability to accurately predict a class). We want to maximize the area under the ROC curve which occurs at the top left of Figure 6. Here the True positive rate is 1 and the false positive rate is 0. An AUC of 1.0 is a perfect model, where the classifier never makes a mistake.
...
The area under this Logistic Classifier based ROC curve is 79%, which means we make a mistake 21% of the time when classifying clients as 1 or 0. This is better than random guessing or employing the Dummy Classifier. Running a 10 fold cross-validation across the sample, drops the area under the curve to 76.1%. 
...
Here is where use of a Logistic Classifier provides us with a comprehensible result. Looking at these coefficients, we can say that more of features A6-10 and A13 we have the more likely the client will fall into Class 1.
...
I run a hyperparameter tuned 10-Fold KNN Classifer on the dataset and discover the optimal number of neighbors is 25! This means that knowing a particular datapoint and the 25 points closest to it gives us a higher likelihood of being in the same group. Unfortunately the Area under the ROC curve is lower than the Logistic Classifier as shown in Figure 8, meaning that it is less accurate at correctly predicting client Class.

<p align="center">
Figure 8
</p>
![_config.yml]({{ site.baseurl }}/images/bcgdvtworoc.png) 

**Final Thoughts: Alternative approaches of tackling the imbalanced class problem**

A simple logistic classifier correctly sorts clients into Class 1 or 0 79% of the time while a K Nearest Neighbors Classifier only gets this right 70% of the time. I would be interested in exploring how a Random Forest Classifier or a Boosted Decision tree performed on this dataset but this woulc come at the cost of comprehensibility. 
