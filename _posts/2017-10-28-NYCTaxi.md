...
A Least Squares Regression model that uses trip time, distance, speed, tip amount and tolls amount to predict fare amount has an R-square higher than 95%. I show a line of best fit among the fare data in Figure 12 below. 
...
As I've excluded "out of town" fares i.e. any trips to Nassau county or Westchester, my analysis will focus on all trips within New York city and its airports. Two out of town fares on the same day (negotiated by the taxi driver) may be higher than the average daily earnings of all taxi drivers.

To determine how a taxi owner can maximize earnings on a given day, I focus on the constraining factor - time. By concentrating on trips that maximize earnings per hour assuming each taxi is driven by two drivers running 12 hour shifts.
...
Given a limited number of taxis, it makes sense to focus on those fares which have the highest earnings per kilometer or hour traveled.
