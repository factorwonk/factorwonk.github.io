...
Interestingly, these trips begin and end at the same geocode (rounded to 2 d.p.) and are all intra-Manhattan or intra-Airport trips, indicating that these shorter trips can be used to maximize daily earnings. The results when trying to maximize the total amount earned per mile driven are identical.
...
Assume each taxi can be driven all day by 2 drivers working 12 hour shifts. This translates to 20 shifts per day for the taxi company. I would ensure taxis are available at the most popular pickup and dropoff locations and for trips with the most consistent fares. However you wouldn't want taxis working for the same company to undercut each other for the same fare.

There could be several approaches to this problem: 

a. Instruct taxi drivers to focus on the routes with the highest earnings per hour. This would keep taxis working within smaller areas (zipcodes) and would allow the company to keep a fleet of cars working airport shifts and another fleet working Manhattan island shifts.
b. 
